...
async def operator_edit(request: Request, session: SessionDep,
    ...
    if auth_data and not auth_data[0]["admin"]:
        ...
        statement = select(Promises).where(Promises.new_name == promise.new_name).where(Promises.new_description == promise.new_description).where(Promises.new_amount == promise.new_amount).where(Promises.new_buy_cost == promise.new_buy_cost).where(Promises.new_kind == promise.new_kind)
        result = await session.execute(statement)
        ...
        if copy is None:
            ...
            await session.commit()
            return RedirectResponse(url='/things', status_code=303)
        return {"error":"already at db", "message": "this promise already at database. Ваше обещание уже существует"}
...
async def admin_requests(request: Request, session: SessionDep):
    ...
    if auth_data and auth_data[0]["admin"]:
        ...
        auth_data = await authenticate_user(request, "admin_requests.html", data=data, now=datetime.now())
        await session.commit()
        return auth_data[1]
...
async def update_time(
    ...
    session: SessionDep,
    id: int,
    days: int = Form(None),  # Необязательное поле из формы
    data: dict = Body(None)   # Необязательное поле из JSON
):
    auth_data = await authenticate_user(request, "admin_requests.html")
    
    if auth_data and auth_data[0]["admin"]:
        # Определяем, откуда взять days
        if days is not None:
            days_value = days
        ...
        promise = await session.get(Promises, id)
        if promise:
            promise.die_at = datetime.now() + timedelta(days=days+1)
...
async def users(request: Request, session: SessionDep):
    ...
    if auth_data and auth_data[0]["admin"]:
        ...
        auth_data = await authenticate_user(request, "users.html", current_user=current_user, users=users)
...
async def delete_item(request: Request, session: SessionDep, id: int):
    ...
    if auth_data and auth_data[0]["admin"]:
        # 1. Добавляем await!
        thing = await session.get(Users, id)
